...
class MY_Controller extends MX_Controller {
	...
	protected $mViewData = array();
		...
		// Mobile
	protected $mMobileVersion = FALSE;
	...
	// Setup values from file: config/ucpw.php
	private function _setup()
	{
		$config = $this->config->item('ucpw');
	...
	private function _presetup()
	{
		$config = $this->config->item('ucpw');
		
		// multilingual setup
		$mobile_config = empty($config['mobile_version']) ? array() : $config['mobile_version'];
		if ( !empty($mobile_config) )
		{		
			$this->mMobileVersion = TRUE;
			$this->_mobile_detect();			
		}
		//setup charset
		$this->mViewData['charset'] = $this->config->item('charset');
		//detect language if not multilanguage
		if (!isset($language)) $this->mViewData['language'] =  $this->mLanguage;
		// add icon
		$this->mViewData['icon_path'] = empty($config['icon']) ? '' : $config['icon'];
		//if ( !empty($icon_path) ){$this->mViewData['icon_path'] = $icon_path; }
		
		$this->mViewData['assets_dir'] = $this->config->item('assets_dir');
		$this->mViewData['plugins_dir'] = $this->config->item('plugins_dir');
		$this->mViewData['frameworks_dir'] = $this->config->item('frameworks_dir');
		
		$this->load->model('prefs_model');
		$this->mViewData['admin_prefs'] = $this->prefs_model->admin_prefs();
		$this->load->helper('menu'); //Function for menu
		$this->lang->load('general');
		$this->mViewData['title_mini']  = empty($config['title_mini']) ? '' : $config['title_mini'];
		$this->mViewData['title_lg']  = empty($config['title_lg']) ? '' : $config['title_lg'];
	}
	
	private function _mobile_detect()
	{
		$this->load->library('mobile_detect');
		 /* Any mobile device (phones or tablets) */
        if ($this->mobile_detect->isMobile())
        {
            $this->mViewData['is_mobile'] = TRUE;

            if ($this->mobile_detect->isiOS()){
                $this->mViewData['is_ios']     = TRUE;
                $this->mViewData['is_android'] = FALSE;
            }
            else if ($this->mobile_detect->isAndroidOS())
            {
                $this->mViewData['is_ios']     = FALSE;
                $this->mViewData['is_android'] = TRUE;
            }
            else
            {
                $this->mViewData['is_ios']     = FALSE;
            ...
            if ($this->mobile_detect->getBrowsers('IE')){
                $this->mViewData['is_mobile_ie'] = TRUE;
            }
            else
            {
                $this->mViewData['mobile_ie'] = FALSE;
            }
        }
        else
        {
            $this->mViewData['is_mobile']    = FALSE;
            $this->mViewData['is_ios']       = FALSE;
            $this->mViewData['is_android']   = FALSE;
            $this->mViewData['is_mobile_ie'] = FALSE;
        }
	}
	...
	protected function render_json($data, $code = 200)
	{
		...
		// Output JSON string
	protected function render_ajax($view_file, $layout = 'ajax')
	{
		// automatically generate page title
		if ( empty($this->mPageTitle) )
		{
